...
public class EssentialsAfk implements Listener
{
	...
	public void onPlayerAfkChange(AfkStatusChangeEvent e)
	{
		...
		if(!e.getAffected().isAfk())
		{
			...
			{
				DiscordSRV.getPlugin().getDestinationTextChannelForGameChannelName(DiscordSRVUtils.BotSettingsconfig.getChatChannel()).sendMessage(conf.getConfigWithPapi(e.getAffected().getBase().getUniqueId(), String.join("\n", DiscordSRVUtils.Config.EssentialsAfkMessage()))
