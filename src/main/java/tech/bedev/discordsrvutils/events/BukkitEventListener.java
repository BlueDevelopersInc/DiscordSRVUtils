...
public class BukkitEventListener implements Listener
{
	...
	public void onChat(AsyncPlayerChatEvent e)
	{
		...
		Bukkit.getScheduler().runTask(core, () ->
		{
			...
			{
				person.clearXP();
				PlayerLevelEvent ev = new PlayerLevelEvent(person, e.getPlayer());
				Bukkit.getPluginManager().callEvent(ev);
				if(!ev.isCancelled())
				{
					person.addLevels(1);
					e.getPlayer().sendMessage(conf.convertToColourCode(conf.getConfigWithPapi(e.getPlayer().getUniqueId(), String.join("\n", DiscordSRVUtils.Levelingconfig.levelup_minecraft()))).replace("[Level]", person.getLevel() + ""));
				}
			}
		});
