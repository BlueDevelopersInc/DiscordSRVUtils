...
public class JDAEvents extends ListenerAdapter
{
	...
	public void onGuildMemberJoin(GuildMemberJoinEvent e)
	{
		...
		Bukkit.getScheduler().runTask(core, () ->
		{
			...
			{
				UUID pluginUUID = DiscordSRV.getPlugin().getAccountLinkManager().getUuid(e.getUser().getId());
				if(pluginUUID != null)
				{
					String pluginName = Bukkit.getOfflinePlayer(pluginUUID).getName();

					if(PunishmentManager.get().isBanned(UUIDManager.get().getUUID(pluginName)))
					{
					...
					else if(PunishmentManager.get().isMuted(UUIDManager.get().getUUID(pluginName)))
					{
						if(DiscordSRVUtils.BansIntegrationconfig.isSyncPunishmentsWithDiscord())
						{
							e.getGuild().addRoleToMember(e.getMember(), e.getGuild().getRoleById(DiscordSRVUtils.Moderationconfig.getMutedRole())).queue();
						}
					}
				}
			}
	...
	public void onGuildMessageReceived(GuildMessageReceivedEvent e)
	{
		...
		String prefix = DiscordSRVUtils.BotSettingsconfig.getBotPrefix();
			...
			else
			{
				if(!(args.length >= 3))
				{
					try
					{
						...
						Member toBan = e.getGuild().getMemberById(args[1]);
						if(toBan == null)
						{
						...
						{
							if(!e.getGuild().getSelfMember().canInteract(toBan))
							{
								e.getChannel().sendMessage("Unable to ban Member Because his role is higher than me.").queue();
								return;
							}
							UUID mcUUID = DiscordSRV.getPlugin().getAccountLinkManager().getUuid(toBan.getId());
							if(mcUUID != null)
							{
							}
							e.getGuild().ban(toBan, 0, "Banned by " + e.getMember().getUser().getAsTag()).queue();
							EmbedBuilder embed = new EmbedBuilder();
							embed.setColor(Color.GREEN);
							embed.setDescription("**_" + toBan.getUser().getAsTag() + " Was banned._**");
							e.getChannel().sendMessage(embed.build()).queue();
						}
					...
					catch(NumberFormatException ex)
					{
						...
						else
						{
							...
							Member toBan = e.getMessage().getMentionedMembers().get(0);
							if(!e.getGuild().getSelfMember().canInteract(toBan))
							{
								e.getChannel().sendMessage("Unable to ban Member Because his role is higher than me.").queue();
								return;
				...
				else
				{
					...
					try
					{
						...
						else
						{
							...
							e.getGuild().ban(toBan, 0, "Banned by " + e.getMember().getUser().getAsTag() + " (" + reason + ")").queue();
							EmbedBuilder embed = new EmbedBuilder();
							embed.setColor(Color.GREEN);
							embed.setDescription("**_" + toBan.getUser().getAsTag() + " Was banned._**");
							e.getChannel().sendMessage(embed.build()).queue();
			...
			else
			{
				try
				{
					...
					e.getGuild().retrieveBanById(args[1]).queue(success ->
					{
						...
						e.getGuild().unban(Long.toString(membertounbanid)).queue();
						EmbedBuilder embed = new EmbedBuilder();
						embed.setColor(Color.GREEN);
			...
			if(isModerator(e.getMember()))
			{
				...
				else if(args.length >= 2)
				{
					try
					{
						...
						Role mutedrole = e.getGuild().getRoleById(DiscordSRVUtils.Moderationconfig.getMutedRole());
						...
						if(mutedrole == null)
						{
							...
							core.getLogger().severe("Role not found on Guild \"" + e.getGuild().getName() + "\" Role ID: " + DiscordSRVUtils.Moderationconfig.getMutedRole());
						}
							...
							if(r2.getInt("Type") != 0)
							{
								...
								else if(r2.getInt("Type") == 5)
								{
									...
									else
									{
										...
										{
											try(Connection connection = core.getDatabaseFile())
											{
												...
												ResultSet mr2 = mp2.executeQuery();
												mr2.next();
												PreparedStatement p6 = connection.prepareStatement("UPDATE discordsrvutils_tickets SET ChannelID=?, MessageID=? WHERE TicketID=?");
			...
			if(!DiscordSRVUtils.BotSettingsconfig.isBungee())
			{
				if(DiscordSRVUtils.Levelingconfig.Leveling_Enabled())
				{
					...
					if(person.isLinked())
					{
						...
						Long val = core.lastChattedAt.get(person.getMinecraftUUID());
						if(val == null)
						{
							core.lastChattedAt.put(person.getMinecraftUUID(), System.nanoTime());
						}
						else
						{
							if(!(System.nanoTime() - val >= EXPIRATION_NANOS)) return;
							core.lastChattedAt.remove(person.getMinecraftUUID());
							core.lastChattedAt.put(person.getMinecraftUUID(), System.nanoTime());
						}
						...
						if(!rr1.next())
						{
							...
							e.getGuild().getCategoryById(r1.getLong("Opened_Category")).createTextChannel("opened-" + e.getMember().getEffectiveName()).queue(channel ->
							{
								...
								try(Connection conn1 = core.getDatabaseFile())
								{
									...
									cr1.next();
									embed.setDescription("here is your ticket.\nReact with \uD83D\uDD12 to close this ticket. or use `" + DiscordSRVUtils.BotSettingsconfig.getBotPrefix() + "close`.\n\n**TicketName:** " + cr1.getString("Name"));
								}
								catch(SQLException ex)
								{
									ex.printStackTrace();
								...
								channel.sendMessage(embed.build()).queue(message2 ->
								{
									...
									try(Connection connection = core.getDatabaseFile();)
									{
										...
										PreparedStatement fp1 = connection.prepareStatement("INSERT INTO discordsrvutils_Opened_Tickets (UserID, MessageID, TicketID, Channel_id) VALUES (?, ?, ?, ?)");
										PreparedStatement fp2 = connection.prepareStatement("SELECT * FROM discordsrvutils_tickets WHERE MessageId=?");
						...
						else
						{
							...
							try(Connection conn5 = core.getDatabaseFile();)
							{
								...
								else
								{
									...
									String remote_yes;
									String remote_no;
									String emoteYes = EmojiParser.parseToUnicode(":" + emote_yes + ":");
									if(!emoteYes.equals(":" + emote_yes + ":"))
									{
										remote_yes = emoteYes;
										emote_yes = emoteYes;
									}
									...
									else
									{
										...
										if(r5.next())
										{
											...
											{
												e.getChannel().retrieveMessageById(e.getMessageId()).queue(msg -> msg.removeReaction(remote_no, e.getUser()).queue());
											}
											else if(e.getReactionEmote().getName().equals(emote_no))
											{
												e.getChannel().retrieveMessageById(e.getMessageId()).queue(msg -> msg.removeReaction(remote_yes, e.getUser()).queue());
